...
from .models import UUID_DEF,ProjectType,Phase,ActivityCode
from .forms import TypeForm,PhaseForm,ActivityCodeForm
...
def phase_jsondata():
    ...
    table.searchable(lambda qs, sq: qs.filter(or_(Phase.name.contains(sq) , Phase.desc.contains(sq))))
    return json.dumps(table.json())
